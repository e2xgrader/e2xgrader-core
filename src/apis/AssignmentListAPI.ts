...
export namespace AssignmentListAPI{
  ...
  export async function fetchAssignments(courseId: string): Promise<INbGraderAssignment[]> {
    const settings = ServerConnection.makeSettings();
  ...
  export async function submitAssignment(courseId: string, assignmentId: string): Promise<IE2xGraderSubmissionResponse>{
    const settings = ServerConnection.makeSettings();
