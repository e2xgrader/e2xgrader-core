...
import {NbgraderCellType, NbgraderCellTypes, NbgraderMetadata} from "./model/nbgrader";
...
export type E2xGraderSharedCell = Omit<SharedCell.Cell, 'metadata'> & {cell_type: string, metadata: IE2xGraderMetadata};

export class CellPresets {
    public static getCleanMetadata(nbgraderCellType: NbgraderCellType, {e2xgraderCellType, linkTargetCellId, taskName, points}: {e2xgraderCellType?: string; linkTargetCellId?: string; taskName?: string, points?: number} = {}):  Partial<IE2xCellMetadata> {
        return {
    ...
    public static getTaskDescriptionPreset(linkTargetCellId?: string): E2xGraderSharedCell {
        return {
            cell_type: TASK_DESCRIPTION_DEFAULT_CELL_TYPE,
            metadata: this.getCleanMetadata(NbgraderCellType.DESCRIPTION, {linkTargetCellId})
        }
    }

    public static getAutograderTestPreset(linkTargetCellId?: string): E2xGraderSharedCell {
        return {
            cell_type: AUTOGRADER_TEST_DEFAULT_CELL_TYPE,
            metadata: this.getCleanMetadata(NbgraderCellType.AUTOGRADER_TEST, {linkTargetCellId})
        }
    }
}